...
foreach pid $main_ps {
    ...
    ns_log Notice "unconfigure: project_id=$pid"
	...
	foreach phase_tuple $phases {
	    ...
	    ns_log Notice "unconfigure: checking: phase_id=$phase_id, phase_start_date=$phase_start_date, date=$date, phase_end_date=$phase_end_date"
#	    if {$date < $phase_start_date} { continue }
#	    if {$date > $phase_end_date} { continue }
	    ns_log Notice "unconfigure: writing: phase_id=$phase_id, phase_start_date=$phase_start_date, date=$date, phase_end_date=$phase_end_date"
	...
	if {$date in $rand_dates} {
	    ...
	    ns_log Notice "unconfigure: delaying project: phase=$delay_phase_id, date=$date, delay_days=$delay_days"
